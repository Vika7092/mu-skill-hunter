...
搜索引擎并行查询全部4个源，去重后以统一表格格式展示，来源标签区分（🐙GitHub / 🦀ClawHub / 🐉SkillHub / 🛠️Skills.sh）。
...
| 统一评分 | 热度 + 相关性 + 新鲜度 | 无 | 无 | 无 |
| 周报 | Cron自动 + 精选Top 8 | 手动 | 无 | 无 |
| 并行搜索 | 四源同时 | 串行 | 单源 | 单源 |
...
| 📊 周报推送 | 每周热门Skill摘要 | Cron（周日18:00）或手动 `trending.py --period weekly` |
...
> 四源发现、12条安全规则、1个统一评分——放心搜索、审查、安装 AI Agent Skill。
